...
Error Remove_File_Alias(sqlite3 **db, char* file_name){
  char sql[64 + strlen(file_name)];
  snprintf(sql, sizeof(sql), "DELETE FROM FileAliases WHERE FileName = '%s'", file_name);
  char *errmsg;
  int status = sqlite3_exec(*db, sql, NULL, NULL, &errmsg);
  if(status != SQLITE_OK){
    printf("ERROR CODE: %s\n", errmsg);
